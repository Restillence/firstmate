...
| `fm-wedge-alarm-lib.sh`  | Single owner of the away-mode wedge alarm's active-alert channels, their per-machine deliverability, and how each one fires ([wedge-alarm.md](wedge-alarm.md)) |
...
| `fm-classify-lib.sh`     | Shared wake-classification vocabulary, durable keyed-decision folds and scans, and the newline-safe status append every writer uses |
