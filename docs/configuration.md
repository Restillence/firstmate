...
FM_SESSION_START_STATUS_TAIL=5   # non-blank state/*.status lines printed per task in the session-start digest; blank separators never consume a slot and each line is capped by bin/fm-line-cap-lib.sh
